...
class Setup(object):
    ...
    def commit(self, values: Polynomial) -> G1Point:
        ...
        # print("before fft", values.values)
        coeffs = values.ifft().values
        # print("after fft", coeffs)
        if len(coeffs) > len(self.powers_of_x):
            raise Exception("Not enough powers in setup")
        print(ec_lincomb([(s, x) for s, x in zip(self.powers_of_x, coeffs)]))
        print("ec linccomb")
    ...
    def verification_key(self, pk: CommonPreprocessedInput) -> VerificationKey:
        ...
        # TODO understand why 5 is used for primitive root
        # TODO what is group order
